...
from inventory.models import product  # Assuming your product model is named `Product`
from django.contrib.auth import authenticate, login, logout
from django.contrib.auth.decorators import login_required
from django.conf import settings
from django.shortcuts import render, redirect
...
def get_daily_profit(date=None):
    """
    Calculate the total profit made per day.
    Profit is calculated as (Selling Price - Cost Price) * Quantity Sold.
    :param date: The specific date to calculate profit for (defaults to today if not provided).
    :return: Total profit for the day.
    """
    if date is None:
        date = datetime.today().date()
    
    transactions = productTransaction.objects.filter(transaction_date_time__date=date)
    
    total_profit = transactions.aggregate(
        total_profit=Sum((F('sales_price') - F('cost_price')) * F('qty'))
    )['total_profit'] or 0  # Return 0 if there are no transactions
    
    return round(total_profit, 2)

# Example usage:
# print(get_daily_profit("2025-02-04"))
# Form to handle barcode or product name search
class EnterBarcode(forms.Form):
    search = forms.CharField(widget=forms.TextInput(attrs={'autofocus':"autofocus",' autocomplete':"off",'style':"width:100%"}), max_length=100)
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Check if product exists based on barcode or product name
            try:
                # Try to find product by barcode first
                product_instance = product.objects.get(barcode=search)
            except product.DoesNotExist:
                # If product with barcode not found, search by product name
                try:
                    product_instance = product.objects.get(name__iexact=search)
                except product.DoesNotExist:
                    # Handle case when product does not exist
                    return redirect("/register/?error=ProductNotFound")

            return redirect(f"/cart/add/{product_instance.barcode}/{qty}")

    # Check if request is an AJAX request (using the `X-Requested-With` header)
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        search_query = request.GET.get('query', '').lower()
        products = product.objects.filter(name__icontains=search_query)
        suggestions = [{"id": product.id, "name": product.name, "barcode": product.barcode} for product in products]
        return JsonResponse(suggestions, safe=False)

    # Existing cart and display logic
    try:
    ...
    context = {
        'form': form,
        'no_product': "ProductNotFound" in request.GET.get('error', ''),
        'cart': cart,
        'total': Total,
        'tax_total': Tax_Total,
                    ...
                    <div class="card-header p-0" >
                        <table class="table p-0 m-0" style="text-align:right;">
                            <tr>
                                ...
                                <th style="font-family: bold;color:rgba(0, 0, 0, 0.623)">Total</th>
                            </tr>
                        </table>
                    </div>
